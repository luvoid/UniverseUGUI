...
namespace UniverseLib.UI
{
    public sealed partial class UIFactory
    {
        ...
        /// <returns>A <see cref="LabelModel"/> wrapper for your <see cref="Text"/> component.</returns>
        public LabelModel Label(GameObject parent, string name, string text, IReadOnlyFrameStyle style = null)
        {
            style ??= (IReadOnlyFrameStyle)Skin?.Label ?? UISkin.Default.Label;

            LabelModel label = new(parent, name, text);
        ...
        /// <returns>A <see cref="ButtonModel"/> wrapper for your <see cref="UnityEngine.UI.Button"/> component.</returns>
        public ButtonModel Button(GameObject parent, string name, string text, IReadOnlyButtonStyle style = null)
        {
            style ??= (IReadOnlyButtonStyle)Skin?.Button ?? UISkin.Default.Button;

            ButtonModel button = new(parent, name, text);
        ...
        /// <returns>A <see cref="ToggleModel"/> wrapper for your <see cref="UnityEngine.UI.Toggle"/> component.</returns>
        public ToggleModel Toggle(GameObject parent, string name, string text, IReadOnlyToggleStyle style = null)
        {
            style ??= (IReadOnlyToggleStyle)Skin?.Toggle ?? UISkin.Default.Toggle;

            ToggleModel toggle = new(parent, name, text);
        ...
        public InputFieldModel InputField(GameObject parent, string name, string placeHolderText, IReadOnlyInputFieldStyle style = null)
        {
            style ??= (IReadOnlyInputFieldStyle)Skin?.InputField ?? UISkin.Default.InputField;

            InputFieldModel inputField = new(parent, name, placeHolderText);
        ...
        /// <returns>A <see cref="ButtonModel"/> wrapper for your <see cref="Button"/> component.</returns>
        public DropdownModel Dropdown(GameObject parent, string name, Action<int> onValueChanged, IReadOnlyDropdownStyle style = null,
            int defaultValue = 0, params string[] defaultOptions)
        {
            style ??= (IReadOnlyDropdownStyle)Skin?.Dropdown ?? UISkin.Default.Dropdown;

            DropdownModel dropdown = new(parent, name, defaultValue, defaultOptions);
